WebCore-Docker is a set of docker images derive from [Docker-LAMP][dockerlamp] images that include the phusion baseimage (16.04 and 18.04 varieties), along with a LAMP stack ([Apache][apache], [MySQL][mysql] and [PHP][php]) and specific package for WebCore Development Environment.
...
## Introduction
These Docker images are specifically intended to assist our team in developing applications using our internal framework.

We used the [Docker-LAMP][dockerlamp] base image by adding a few small setups to suit our needs.

## Image Versions
There are 2 main 'versions' of the docker image. The table below shows the different tags you can use, along with the PHP, MySQL and Apache versions that come with it.
...
docker run -p "80:80" -v ${PWD}/app:/app kubuslab/webcore:latest-1804

# Launch a 16.04 based image
docker run -p "80:80" -v ${PWD}/app:/app kubuslab/webcore:latest-1604
```

### With a Dockerfile
```docker
FROM kubuslab/webcore:latest-1804
...
##### PHPMyAdmin
WebCore-Docker comes pre-installed with phpMyAdmin available from `http://DOCKER_ADDRESS/phpmyadmin`.
...
### Adding your app
The below command will run the docker image `kubuslab/webcore:latest` interactively, exposing port `80` on the host machine with port `80` on the docker container. It will then create a volume linking the `app/` directory within your project to the `/app` directory on the container. This is where Apache is expecting your PHP to live.
```bash
docker run -i -t -p "80:80" -v ${PWD}/app:/app kubuslab/webcore:latest
```

### Persisting your MySQL
The below command will run the docker image `kubuslab/webcore:latest`, creating a `mysql/` folder within your project. This folder will be linked to `/var/lib/mysql` where all of the MySQL files from container lives. You will now be able to stop/start the container and keep your database changes.
...
```bash
docker run -i -t -p "3306:3306" -v ${PWD}/mysql:/var/lib/mysql kubuslab/webcore:latest
```
...
```bash
docker run -i -t -p "80:80" -p "3306:3306" -v ${PWD}/app:/app -v ${PWD}/mysql:/var/lib/mysql kubuslab/webcore:latest
```

#### `.bash_profile` alias examples
The below example can be added to your `~/.bash_profile` file to add the alias commands `ldi` and `launchdocker`. By default it will launch the 16.04 image - if you need the 14.04 image, simply change the `docker run` command to use `kubuslab/webcore:latest-1404` instead of `kubuslab/webcore:latest`.
```bash
# A helper function to launch docker container using kubuslab/webcore with overrideable parameters
#
...
function launchdockerwithparams {
    ...
    docker run -i -t -p "$APACHE_PORT:80" $MYSQL_PORT_COMMAND -v ${PWD}/app:/app -v ${PWD}/mysql:/var/lib/mysql kubuslab/webcore:latest
}
...
git clone https://github.com/kubuslab/docker-webcore.git
cd docker-webcore

# Build the images
docker build -t=kubuslab/webcore:latest -f ./1804/Dockerfile .
docker build -t=kubuslab/webcore:latest-1604 -f ./1604/Dockerfile .
docker build -t=kubuslab/webcore:latest-1804 -f ./1804/Dockerfile .

# Run the image as a container
docker run -d -p "3000:80" kubuslab/webcore:latest-1804
...
## Inspiration
This image was originally based on [mattrayner/docker-lamp][dockerlamp], with a few changes to make it compatible with the WebCore Environment Development.


## License
WebCore-Docker is licensed under the [Apache 2.0 License][info-license].
...
[info-build-status]: https://circleci.com/gh/kubuslab/docker-webcore
[info-docker-hub]: https://hub.docker.com/r/kubuslab/webcore
